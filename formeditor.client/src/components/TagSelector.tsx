...
export default function TagSelector(props: TagSelectorProps) {
    ...
    const [allTags, setAllTags] = createSignal(['job', 'application', 'hiring', 'interview', 'recruitment']);
    ...
    return (
        <div>
            <label htmlFor="tags" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Tags</label>
            <Combobox<string>
                multiple
                options={tagOptions()}
                value={props.tags}
                ...
                itemComponent={props => allTags().includes(props.item.rawValue) || props.tags.includes(props.item.rawValue)
                    ?
                    <ComboboxItem item={props.item} class="p-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700">
                    ...
                    : <ComboboxItem item={props.item}
                        ...
                        <ComboboxItemLabel>Create new tag '<b>{props.item.rawValue}</b>'</ComboboxItemLabel>
                        <ComboboxItemIndicator>
                            <FaSolidCheck/>
                        </ComboboxItemIndicator>
                    </ComboboxItem>
